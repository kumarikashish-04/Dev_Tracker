...
export default function Settings() {
  ...
  const handleConnectRepo = async (e) => {
    ...
    try {
      ...
      setRepo('');
      await fetchRepos();
      // Redirect straight to dashboard for the synced repo
      navigate(`/dashboard?owner=${data.repoOwner}&repo=${data.repoName}`);
    } catch (err) {
      setSyncError(err.message || 'Sync failed. Check credentials, repository path, or network rate limits.');
    } finally {
  ...
  const handleDeleteRepo = async (ownerName, repoName) => {
    if (!window.confirm(`Disconnect ${ownerName}/${repoName} and delete all stored analysis?`)) return;
    try {
    ...
    <div className="max-w-7xl mx-auto px-6 py-12 flex-1 w-full space-y-10">
      <div>
        ...
        <p className="text-slate-400 mt-2 text-base">
          Configure your GitHub API keys and connect repositories to trigger AI insights
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-8">
          ...
          <div className="glass-panel p-6 rounded-2xl border border-slate-800/80 shadow-xl space-y-5">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 bg-slate-900 rounded-xl flex items-center justify-center border border-slate-800">
                <svg className="w-5.5 h-5.5 text-slate-300" fill="currentColor" viewBox="0 0 24 24">
              ...
              <div>
                <h2 className="text-lg font-heading font-bold text-white">GitHub API Token</h2>
                <p className="text-[11px] text-slate-400 font-semibold tracking-wide uppercase">Credentials</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSaveToken} className="space-y-4">
              ...
              <div className="bg-slate-900/40 border border-slate-800/80 p-3 rounded-xl">
                <span className="block text-[11px] font-bold text-sky-400 uppercase mb-1">💡 Token Requirements</span>
                <p className="text-[11px] text-slate-400 leading-relaxed">
                  Generate a **Personal Access Token (Classic)** on GitHub with the <code className="text-slate-300 bg-slate-800 px-1 rounded">repo</code> scope enabled so DevTrackr can scan repository commits, branches, and issue backlogs.
                </p>
              </div>
              ...
              >
                {tokenLoading ? 'Saving...' : tokenSuccess ? '✓ Token Saved' : 'Save Connection Token'}
              </button>
            </form>
          </div>
          ...
          <div className="glass-panel p-6 rounded-2xl border border-slate-800/80 shadow-xl space-y-5">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 bg-slate-900 rounded-xl flex items-center justify-center border border-slate-800">
                <svg className="w-5.5 h-5.5 text-sky-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4"></path>
                </svg>
              </div>
              <div>
                <h2 className="text-lg font-heading font-bold text-white">Sync Repository</h2>
                <p className="text-[11px] text-sky-400 font-semibold tracking-wide uppercase">New Connection</p>
              </div>
            </div>
            ...
            <form onSubmit={handleConnectRepo} className="space-y-4">
              ...
              <div className="bg-slate-900/40 border border-slate-800/80 p-3 rounded-xl text-[11px] text-slate-400 space-y-1">
                <span className="block font-bold text-indigo-400 uppercase">🚀 Try Demo Mode</span>
                <p>
                  No token or key? Input Owner <code className="text-slate-300 font-mono font-bold">demo</code> and Name <code className="text-slate-300 font-mono font-bold">devtracker</code> (or <code className="text-slate-300 font-mono font-bold font-semibold">react</code>) to load simulated data instantly!
                </p>
              </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="glass-panel rounded-2xl border border-slate-800/80 shadow-xl overflow-hidden">
            <div className="px-6 py-5 border-b border-slate-800/80 flex items-center justify-between">
              <div>
                <h2 className="text-lg font-heading font-bold text-white">Connected Repositories</h2>
                <p className="text-slate-400 text-xs mt-0.5">Active analytical instances associated with this dashboard</p>
              </div>
              <button 
                onClick={fetchRepos}
            ...
            {reposLoading ? (
              ...
              <div className="p-12 text-center text-slate-500 space-y-3">
                ...
                <div className="text-sm font-semibold text-slate-400">No repositories connected yet</div>
                <p className="text-xs text-slate-500 max-w-sm mx-auto">Use the connection panel on the left to sync your first repository using a GitHub PAT or try out Demo Mode.</p>
              </div>
            ) : (
              <div className="divide-y divide-slate-800/80">
                {connectedRepos.map((r) => (
                  <div key={r._id} className="p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:bg-slate-900/20 transition-colors">
                    <div className="space-y-1">
                      ...
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-slate-400 text-xs">
                        <span>Commits: <strong className="text-slate-300 font-semibold">{r.metrics?.commitCount || 0}</strong></span>
                        <span>Open PRs: <strong className="text-slate-300 font-semibold">{r.metrics?.openPRCount || 0}</strong></span>
                        <span>Open Issues: <strong className="text-slate-300 font-semibold">{r.metrics?.openIssueCount || 0}</strong></span>
                      </div>
                    ...
                    <div className="flex items-center gap-3.5 shrink-0 self-end sm:self-center">
                      ...
                      <button
                        onClick={() => handleDeleteRepo(r.repoOwner, r.repoName)}
